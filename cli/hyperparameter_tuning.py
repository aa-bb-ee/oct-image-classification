...
OPTUNA_ROOT = PROJECT_ROOT / "experiment_outputs" / "optuna"
...
def _copy_reports(paths, trial_dir: Path) -> None:
    ...
    for src_dir in report_targets:
        if not src_dir.exists():
            continue

        rel_name = src_dir.name
        dst_dir = trial_dir / "reports" / rel_name
...
def _cleanup_trial_output(paths) -> None:
    if paths.output_root.exists():
        shutil.rmtree(paths.output_root, ignore_errors=True)


def objective(args: argparse.Namespace, study_dir: Path):
    ...
    def _objective(trial: optuna.Trial) -> float:
        ...
        with open(trial_dir / "summary.json", "w", encoding="utf-8") as f:
            json.dump(_jsonify(results.summary), f, indent=2, ensure_ascii=False)

        _copy_reports(paths, trial_dir)

        trial.set_user_attr("run_id", paths.run_id)
        trial.set_user_attr("output_root", str(paths.output_root))
...
def main() -> None:
    ...
    if args.mode == "finetune" and not args.base_model_path:
        raise ValueError("--base_model_path is required when --mode finetune is used.")

    from src.gpu import configure_gpu

    configure_gpu(args.gpu_index)

    study_dir = OPTUNA_ROOT / args.study_name
    ...
    study.optimize(objective(args, study_dir), n_trials=args.n_trials)

    best = study.best_trial

    with open(study_dir / "study_summary.json", "w", encoding="utf-8") as f:
        json.dump(
            {
                "study_name": args.study_name,
                "mode": args.mode,
                ...
                "best_attrs": _jsonify(best.user_attrs),
            },
            f,
            indent=2,
            ensure_ascii=False,
    ...
    best_out_dir = study_dir / "best_trial"
    if best_out_dir.exists():
        shutil.rmtree(best_out_dir)
    shutil.copytree(best_trial_dir, best_out_dir)
